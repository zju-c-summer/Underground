        ...
        for(int j=central_x-PAINT_BLOCK_NUMBER;j<central_x+PAINT_BLOCK_NUMBER;j++){
            if(i>=0&&i<maze->get_col_size()&&j>=0&&j<maze->get_row_size()){
               /* if(maze->get_block(i,j).get_type()==0)
                ...
                case 1:pix.load("wall_dark.png"); painter->setBrush(white);painter->drawPixmap(j*maze->get_block(i,j).get_block_size()-maze->get_man().get_x(),i*maze->get_block(i,j).get_block_size()-maze->get_man().get_y(),maze->get_block(i,j).get_block_size(),maze->get_block(i,j).get_block_size(),pix);break;
